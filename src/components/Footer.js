    ...
    <footer data-aos='fade-up'>
      <div className='container mx-auto'>
        <div className='flex flex-col xl:flex-row text-center xl:text-left gap-y-12'>
          ...
          {/* lists */}
          <div className='flex flex-1 flex-col gap-y-14 xl:flex-row justify-end'>
            {/* social list */}
            <div>
              ...
              <ul className='flex gap-y-4 gap-x-4 justify-center'>
                {socialList.map((item, index) => {
                  return (
                    <li
                      className='w-12 h-12 bg-primary/10 flex justify-center items-center rounded-full cursor-pointer hover:bg-[#4169e1] transition-all'
                      key={index}
                    >
                      <a
